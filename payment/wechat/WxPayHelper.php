...
class WxPayHelper
{
	...
	protected function get_cft_package(){
		...
		try {
			...
			return $paraString . "&sign=" . $md5SignUtil->sign($unSignParaString,$commonUtil->trimString($wechat['key']));
	...
	function get_biz_sign($bizObj){
		...
		try {
			$bizParameters["appkey"] = $wechat['signkey'];
			ksort($bizParameters);
