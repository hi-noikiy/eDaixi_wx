...
/*function template($filename, $flag = TEMPLATE_DISPLAY) {
	...
	if (defined('IN_MOBILE')) {
		$source = IA_ROOT."/framework/view/homepage/wap_index.html";
		$compile = IA_ROOT."/data/tpl/homepage/wap_index.tpl.php";
...
function template($filename,$module = null,$act = null){
	if(empty($module)){
		$source = IA_ROOT."/framework/view/wap/base/"."$filename.html";
		$compile = IA_ROOT."/data/tpl/wap/"."$filename.tpl.php";
	}else{
		$source = IA_ROOT."/framework/view/$module/$act/$filename.html";
		//var_dump($source);exit;
		$compile = IA_ROOT."/data/tpl/$module/$act/$filename.tpl.php";
	}
	if(!is_file($source)) {
		var_dump($source);
		echo '<br>';
...
function template_compile_from_str($source_str, $to, $instance = false)
{
	$path = dirname($to);
	if (!is_dir($path))
		mkdirs($path);
	$content = template_parse($source_str, $instance);
	file_put_contents($to, $content);
}
	...
	if (!function_exists($funcname)) {
		if (!isset($includes[$modulename])) {
			if (!file_exists(IA_ROOT . '/source/modules/'.$modulename.'/model.php')) {
				...
				include_once IA_ROOT . '/framework/model/'.$modulename.'/model.php';
			}
		}
	}
